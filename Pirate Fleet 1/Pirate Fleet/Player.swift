...
struct PlayerSeaMonster: PenaltyCell {
    let location: GridLocation
    let guaranteesHit: Bool
    let penaltyText: String
...
class Player {
    ...
    func addPlayerShipsMines(numberOfMines: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            var location = RandomGridLocation()
            var seamonster = PlayerSeaMonster(location: location)
            while !gridViewController.addMine(seamonster, playerType: .Computer) {
                location = RandomGridLocation()
                seamonster = PlayerSeaMonster(location: location)
            }
        }
    }
}